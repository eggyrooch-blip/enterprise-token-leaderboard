...
| 模式 | 适用 | 入口 |
|---|---|---|
| MDM / 飞连（macOS） | 有统一终端管理 | `agent/mdm_bootstrap.sh`（LaunchAgent，下载 `/tokreport.sh`） |
| MDM / 飞连（Windows） | 有统一终端管理 | `agent/mdm_bootstrap_windows.ps1`（Task Scheduler / Scheduled Task，下载 `/tokreport.ps1`） |
| 无 MDM 自助 | 没有 MDM 的企业 | `agent/bootstrap.sh`（`curl\|bash`，免 root，git email 自动归属） |
| 随开发环境捆绑 | 已有 dotfiles/装机脚本 | 把 bootstrap 步骤并进现有装机流程 |

## 弱感知 / 无感知

- macOS 客户端是 LaunchAgent 后台静默跑；Windows 客户端是 Task Scheduler 的 Scheduled Task，跑在登录用户上下文。
- 两端都无弹窗、无交互；日志仅用于本机排障。
- 身份自动解析 → 员工无需任何录入。
