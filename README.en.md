...
> [!NOTE]
> All screenshots use **synthetic mock data** (`zhangsan@example.com`, etc.). No real names,
> avatars, logos, or company information. The open-source repo ships neutral and runnable by default.
...
| | Feature | Notes |
|---|---|---|
| 📈 | **Multi-dimension boards** | Person / Team / Tool / Model / Cursor / Claude / Codex / LiteLLM / Agent, switch window with `?days=7\|30\|90` |
...
| 🟦 | **SaaS AI-quota board** | Pull AI usage from SaaS admin consoles that have **no official API** (e.g. Feishu/Lark AI quota) into the same dashboard as a **separate board** (unit "points", not summed with coding tokens) — see the Feishu collector |
...
| 🪪 | **Pluggable identity** | `EMPLOYEE_EMAIL` (MDM/SSO-pushed) → device serial (collector reverse-lookup) → login\@domain, level by level; optional git-email resolver included (off by default) |
| ♻️ | **Idempotent reporting** | Upsert keyed on `(email, date, source, tool, model)` — backfills / reruns / offline catch-up never double-count |
| 🚀 | **1-minute start** | Zero-dependency SQLite demo: one seed + one start command |
...
open http://localhost:8090/           # ← person/team/tool/model/Cursor/Claude/Codex/LiteLLM/Agent boards + governance metrics + Feishu AI quota
```
...
| SaaS AI quota | `feishu` board | `collector/feishu/` (no official API → copied Chrome profile + CDP side-channel) | daily | Feishu AI quota / trend / per-person usage (unit "points", separate board) |

Each path **looks back a few days idempotently**; the collector upserts by primary key, so
offline catch-up and reruns never double-count.

> **Feishu AI-quota collector** (`collector/feishu/`): Feishu has no official AI-quota usage API, so the
> approach is to copy a logged-in Chrome profile → start headless with a debug port → attach via Playwright/CDP,
> let the page make its own authenticated requests and **capture responses at the network layer** (invisible to
> anti-tamper), normalize, and report over HTTPS — **never touching the DB directly**. See `collector/feishu/README.md`.
...
Identity defaults to the MDM-pushed `EMPLOYEE_EMAIL` or the device serial (resolved by the collector) — zero employee input; orgs without MDM can enable the built-in git-email resolver (`agent/identity.py`, off by default).
...
| Team board | Tool / Model board | LiteLLM board |
|---|---|---|
| ![Team board](docs/img/dashboard-team.png) | ![Tool board](docs/img/dashboard-tool.png) | ![LiteLLM board](docs/img/dashboard-litellm.png) |

| Big-tech governance metrics | Feishu AI quota (separate board, unit "points") |
|---|---|
| ![Governance](docs/img/dashboard-governance.png) | ![Feishu AI quota](docs/img/dashboard-feishu.png) |
